...
def privacy_policy():
    ...
    return public_page(
        ...
        <h2>広告配信サービス</h2>
        <p>
            本サービスでは、将来的に第三者の広告配信サービスを利用し、
            広告を表示する場合があります。
            広告配信サービスは、広告の配信、効果測定、広告の最適化等のため、
            端末情報、広告識別子、IPアドレス、広告の表示・操作に関する情報等を
            取得・利用する場合があります。
        </p>
        <p>
            広告配信サービスを実際に導入する場合は、
            利用する事業者およびその利用方法に応じて、本ポリシーの内容を更新します。
        </p>
...
def terms_of_service():
    ...
    return public_page(
        ...
        <h2>9. 広告</h2>
        <p>
            本サービスでは、サービスの運営・提供等を目的として、
            第三者が提供する広告を表示する場合があります。
            広告の内容、広告主との取引その他広告に関連して生じた事項について、
            TOASTERは、法令上認められる範囲で責任を負いません。
        </p>
